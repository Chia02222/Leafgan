...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        self.image_label = ['real_A', 'real_B', 'fake_A', 'fake_B']  # Replace with your actual image label key
    ...
    def display_current_results(self, visuals, epoch, save_result):
        """
        Display and save results for multiple images to visualize how the model trains.
        ...
        if self.use_html and (save_result or not self.saved):  # Save images to an HTML file
            ...
            if self.fixed_images is None:
                # Cache the images for the selected labels and indices
                self.fixed_images = {label: visuals[label][self.fixed_indices] for label in self.image_labels}

            # Print message showing which image labels are being used
            print(f'Epoch {epoch}: Saving images for labels {", ".join(self.image_labels)}')
