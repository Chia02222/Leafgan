...
if __name__ == '__main__':
    ...
    # Debug: Print the length of the datasets
    dataset_A = create_dataset(opt)  # Assuming this loads trainA
    dataset_B = create_dataset(opt)  # Modify this if separate loading logic is required

    print('Number of images in trainA:', len(dataset_A))
    print('Number of images in trainB:', len(dataset_B))

    # Combine datasets if necessary
    combined_dataset = torch.utils.data.ConcatDataset([dataset_A, dataset_B])
    dataset_size = len(combined_dataset)
    print('Total number of training images:', dataset_size)
